...
function parseListing(text) {
  ...
  // Sol number — handles SPE2DS26T6498 AND SPE2DS-26-T-6498
  const sol = text.match(/\b(SPE[A-Z0-9][A-Z0-9\-]*[A-Z0-9])/i);
  d.sol_number = sol ? sol[1] : null;

  // Qty + unit — handles plain digits AND comma-formatted numbers (18,642)
  const UNITS = 'EA|LT|BX|DZ|PR|FT|GL|LB|PK|RL|SE|ST|PG|HD|SH|VI|CY|GR|MX|TH|YD';
  const qtyRx = new RegExp('\\b(\\d{1,3}(?:,\\d{3})*|\\d+)[ \\t]+(' + UNITS + ')\\b', 'i');
  const qty   = text.match(qtyRx);
  if (qty) { d.quantity = parseFloat(qty[1].replace(/,/g, '')); d.unit_of_issue = qty[2].toUpperCase(); }
  ...
  if (qty && qty.index != null) {
    ...
    if (afterSave) {
      ...
      const raw   = qIdx > 0 ? chunk.substring(0, qIdx).trim() : chunk.split(/[\t]{2,}|\s{3,}/)[0].trim();
      d.item_name = raw.replace(/[\s\d]+$/, '').trim() || null;
    }
  }

  // Prices — try dollar-signed first, fall back to plain decimals
  let prices = [...text.matchAll(/\$([0-9,]+\.\d{2})/g)].map(m => parseFloat(m[1].replace(/,/g, '')));
  if (prices.length === 0) {
    prices = [...text.matchAll(/(?:^|[\t ])(\d+\.\d{2})(?=[\t ]|$)/gm)].map(m => parseFloat(m[1]));
  }
  ...
  const allDates = [...text.matchAll(/(\d{2}\/\d{2}\/\d{2})/g)].map(m => m[1]);
  ...
  if (del) { d.delivery_days = parseInt(del[1]); }
  else {
...
function parseAIText(text) {
  ...
  if (tabLines.length >= 3) {
    ...
    tabLines.forEach(line => {
      ...
      if (tab < 1) return;
      const k = line.slice(0, tab).trim().toLowerCase().replace(/[\s\/]+/g, '_');
      const v = line.slice(tab + 1).trim();
      if (v && !/^not (shown|indicated|specified)/i.test(v)) tbl[k] = v;
    });
    if (tbl.nsn)        { d.nsn = normalizeNSN(tbl.nsn.replace(/[\s\-]/g,'')); d.fsc = d.nsn.slice(0,4); }
    ...
    if (tbl.quantity) {
      const qm = tbl.quantity.match(/^(\d{1,3}(?:,\d{3})*|\d+(?:\.\d+)?)\s*([A-Z]{2})?/i);
      if (qm) { d.quantity = parseFloat(qm[1].replace(/,/g,'')); if (qm[2]) d.unit_of_issue = qm[2].toUpperCase(); }
    }
    if (tbl.unit_price) {
    ...
    if (tbl.packaging) {
      const pkgm = tbl.packaging.match(/(ASTM\s+[A-Z]\d+\w*|MIL-STD-\d+\w*)/i);
  ...
  // ── NSN ─────────────────────────────────────────────────────────────
  const nsnLabeled = text.match(/\*{0,2}NSN[:\*\s]+\*{0,2}([0-9]{4}[\-]?[0-9]{2}[\-]?[0-9]{3}[\-]?[0-9]{4})/i);
  ...
  const itemOneliner = text.match(/SPE\w+\s*[—–-]\s*([A-Z][A-Z,\s]+?)(?:\s*[·\|]|\s*FSC|\s*$)/im);
  if (itemMd)            d.item_name = itemMd[1].trim();
  else if (itemOneliner) d.item_name = itemOneliner[1].trim();

  // ── SUPPLIERS ────────────────────────────────────────────────────────
  const suppMd = [...text.matchAll(/\*\*([^*]+)\*\*\s*[—–-]\s*CAGE\s*\*\*([A-Z0-9]{5})\*\*\s*[—–-]\s*\*\*P\/N\s+([^*\n]+)\*\*/gi)];
  if (suppMd.length > 0) {
    d.ref_supplier      = suppMd[0][1].trim();
    d.ref_supplier_cage = suppMd[0][2].trim();
    d.ref_part_number   = suppMd[0][3].trim();
    d.all_suppliers     = suppMd.map(s => `${s[1].trim()} (${s[2].trim()}) P/N: ${s[3].trim()}`).join(' · ');
  }

  if (!d.ref_supplier) {
    const suppCondensed = [...text.matchAll(/([A-Z][A-Za-z\s]+?)\s*\(([A-Z0-9]{5}),\s*([^)]+)\)/g)];
    if (suppCondensed.length > 0) {
      ...
      d.all_suppliers     = suppCondensed.map(s => `${s[1].trim()} (${s[2].trim()}) P/N: ${s[3].trim()}`).join(' · ');
    }
  }

  if (!d.ref_supplier) {
    const pipeSups = [...text.matchAll(/([A-Z][A-Z0-9 ,\.&]+?)\|([A-Z0-9]{5})\|([^|;\n]+)/g)];
    if (pipeSups.length > 0) {
      d.ref_supplier      = pipeSups[0][1].trim();
      d.ref_supplier_cage = pipeSups[0][2];
      d.ref_part_number   = pipeSups[0][3].trim();
      d.all_suppliers     = pipeSups.map(s => `${s[1].trim()} (${s[2]}) P/N: ${s[3].trim()}`).join(' · ');
    }
  }
  ...
  if (!d.ship_to) {
    ...
    const streetLine = clean.match(/\d+\s+[A-Z][A-Z\s]+(RD|ST|AVE|BLVD|DR|WAY|LN|PKWY|ROAD|STREET|CROCKETT)[^\n]*/i);
    if (streetLine) {
      const idx = clean.indexOf(streetLine[0]);
      const lines = clean.slice(idx).split('\n').slice(0, 3).map(l => l.trim()).filter(Boolean);
      d.ship_to = lines.join(', ');
    }
  }

  if (!d.ship_to) {
    const vessel = text.match(/USS\s+[\w\s]+?(?:CVN|LHD|DDG|LPD|LHA|FFG|CG|SSN)\s*\d+[^\n]*/i);
  ...
  // ── SHELF LIFE ───────────────────────────────────────────────────────
  const sl = text.match(/\b(\d+)[\- ]months?\s+(?:shelf|non)|shelf[\s\-]?life[^:]*?[:\s]+(\d+)\s*months?/i);
