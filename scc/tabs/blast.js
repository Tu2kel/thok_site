...
(254) 265-9335`;
  ...
  function BlastTab() {
    ...
    const handleParse = useCallback(() => {
      ...
      if (!sols.length) {
        setParseError("No valid sol lines found. Format: SOL_ID | ITEM_NAME | DUE | FSC | EXT | STATUS");
        return;
      }
      setParsedSols(sols);
      setFscGroups({});
      setExpandedFsc({});
      setStatus(sols.length + " sols parsed. " + Object.keys(groupByFsc(sols)).length + " FSC lanes. Ready to load distributors.");
    }, [solInput]);
    ...
    const handleLoadDists = useCallback(async () => {
      ...
      for (const fsc of Object.keys(groups)) {
        ...
        try {
          ...
          result[fsc] = { sols: groups[fsc], dists: Array.isArray(dists) ? dists : [] };
        } catch (e) {
          result[fsc] = { sols: groups[fsc], dists: [], error: e.message };
        }
      }
      setFscGroups(result);
      setExpandedFsc(Object.fromEntries(Object.keys(result).map((f) => [f, true])));
      setLoading(false);
      setLoadingFsc("");
      const totalDists = Object.values(result).reduce((s, g) => s + g.dists.length, 0);
      setStatus("Loaded. " + Object.keys(result).length + " lanes · " + totalDists + " distributor contacts ready.");
    }, [parsedSols]);
    ...
    const handleMarkSent = useCallback((fsc, dist, sols) => {
      ...
      refreshLog();
      setStatus("Logged blast to " + dist.name + " covering " + sols.length + " sol(s).");
    }, []);
    ...
    return h(
      ...
      h("div", { style: S.header },
        h("div", null,
          ...
          h("div", { style: S.sub }, "Pre-pipeline sourcing · FSC routing · Distributor RFQ blast"),
        ),
        h("div", { style: S.row },
          h("button", {
            style: { ...S.btn, borderColor: activeView === "blast" ? "rgba(201,168,76,.7)" : undefined, color: activeView === "blast" ? "var(--gold-solid,#C9A84C)" : undefined },
            onClick: () => setActiveView("blast"),
          }, "⚡ Blast"),
          h("button", {
            style: { ...S.btn, borderColor: activeView === "log" ? "rgba(201,168,76,.7)" : undefined, color: activeView === "log" ? "var(--gold-solid,#C9A84C)" : undefined },
            onClick: () => { setActiveView("log"); refreshLog(); },
      ...
      // ── STATUS BAR ──
      status && h("div", {
        style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "#2ecc71", background: "rgba(46,204,113,.08)", border: "1px solid rgba(46,204,113,.2)", borderRadius: "3px", padding: "7px 12px", marginBottom: "14px", letterSpacing: ".04em" },
      }, "▶ " + status),

      // ═══════════════════════════════════════
      //  BLAST VIEW
      // ═══════════════════════════════════════
      activeView === "blast" && h(Frag, null,

        // ── SOL INPUT ──
        h("div", { style: S.card },
          h("div", { style: S.cardTitle }, "Step 1 — Paste Surviving Sols"),
          h("div", { style: { ...S.dim, marginBottom: "8px" } },
          ...
          h("textarea", {
            ...
            placeholder: "SPE4A7-26-R-0001 | BOLT HEX HEAD | 2026-05-15 | 5306 | 12450.00 | GO\nSPE4A7-26-R-0002 | VALVE GATE | 2026-05-20 | 4820 | 8200.00 | GO",
          }),
          parseError && h("div", { style: { color: "#e74c3c", fontFamily: "JetBrains Mono,monospace", fontSize: "10px", marginTop: "6px" } }, "⚠ " + parseError),
          h("div", { style: { ...S.row, marginTop: "10px" } },
            h("button", { style: S.btnPrimary, onClick: handleParse }, "Parse Sols"),
            parsedSols.length > 0 && h("span", { style: S.dim }, parsedSols.length + " sols · " + Object.keys(groupByFsc(parsedSols)).length + " lanes"),
            parsedSols.length > 0 && h("button", { style: S.btnDanger, onClick: () => { setParsedSols([]); setFscGroups({}); setSolInput(""); setStatus(""); } }, "Clear"),
          ),
        ),

        // ── LOAD DISTRIBUTORS ──
        parsedSols.length > 0 && h("div", { style: S.card },
          h("div", { style: S.cardTitle }, "Step 2 — Load Distributor Blast Groups"),
          h("div", { style: { ...S.dim, marginBottom: "10px" } },
          ...
          h("button", {
            ...
            disabled: loading,
          }, loading ? ("Loading " + loadingFsc + "...") : "Load Distributors by FSC"),
        ),

        // ── FSC BLAST GROUPS ──
        Object.keys(fscGroups).length > 0 && h("div", null,
          h("div", { style: { ...S.cardTitle, marginBottom: "12px" } },
            "Step 3 — Review & Fire RFQs"
          ),

          Object.keys(fscGroups).sort().map((fsc) => {
            const { sols, dists, error } = fscGroups[fsc];
            const isOpen = expandedFsc[fsc] !== false;
            const fscLabel = "FSC " + fsc + " — " + (FSC_NAMES[fsc] || fsc);
            const totalExt = sols.reduce((s, d) => s + d.ext, 0);

            return h("div", { key: fsc, style: S.fscSection },

              // FSC header
              h("div", {
                style: { ...S.fscHeader, cursor: "pointer" },
                onClick: () => setExpandedFsc((prev) => ({ ...prev, [fsc]: !isOpen })),
              },
                h("div", { style: S.row },
                  h("span", { style: { fontFamily: "Cinzel,serif", fontSize: "11px", color: "var(--gold-solid,#C9A84C)", letterSpacing: ".1em" } }, fscLabel),
                  h("span", { style: S.badge("rgba(201,168,76,.15)") }, sols.length + " sol" + (sols.length !== 1 ? "s" : "")),
                  h("span", { style: S.badge("rgba(46,204,113,.12)") }, "$" + totalExt.toLocaleString()),
                  dists.length > 0 && h("span", { style: S.badge("rgba(52,152,219,.15)") }, dists.length + " dist" + (dists.length !== 1 ? "s" : "")),
                  error && h("span", { style: { color: "#e74c3c", fontSize: "10px", fontFamily: "JetBrains Mono,monospace" } }, "⚠ " + error),
                ),
                h("span", { style: { color: "rgba(201,168,76,.5)", fontSize: "12px" } }, isOpen ? "▲" : "▼"),
              ),
              ...
              isOpen && h("div", null,
                ...
                // Sol list
                h("div", { style: { padding: "10px 14px", background: "rgba(0,0,0,.2)", borderBottom: "1px solid rgba(201,168,76,.07)" } },
                  h("div", { style: { ...S.dim, marginBottom: "6px" } }, "SOLICITATIONS IN THIS LANE:"),
                  sols.map((sol) =>
                    h("div", { key: sol.id, style: { ...S.row, marginBottom: "4px" } },
                      h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "var(--gold-solid,#C9A84C)", minWidth: "160px" } }, sol.id),
                      h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "rgba(245,240,232,.75)", flex: 1 } }, sol.nom),
                      sol.ext > 0 && h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "#2ecc71" } }, "$" + sol.ext.toLocaleString()),
                    )
                  ),
                ),

                // No distributors state
                dists.length === 0 && h("div", { style: { padding: "16px 14px", color: "rgba(231,76,60,.7)", fontFamily: "JetBrains Mono,monospace", fontSize: "10px" } },
                  "⚠ No distributors loaded for FSC " + fsc + ". Add contacts to the distributor DB first."
                ),
                ...
                dists.map((dist) => {
                  ...
                  return h("div", { key: dist.id, style: S.distCard },
                    h("div", { style: { ...S.row, marginBottom: "8px" } },
                      h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "11px", color: "var(--alabaster,#F5F0E8)", fontWeight: "700", flex: 1 } }, dist.name),
                      h("span", { style: S.badge(dist.tier === "L1" ? "rgba(201,168,76,.3)" : dist.tier === "L3" ? "rgba(231,76,60,.2)" : "rgba(52,152,219,.2)") }, dist.tier || "L2"),
                    ),

                    h("div", { style: { ...S.row, marginBottom: "8px", gap: "16px" } },
                      dist.website && h("a", { href: dist.website, target: "_blank", rel: "noopener", style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "rgba(52,152,219,.8)", textDecoration: "none" } }, dist.website.replace("https://", "")),
                      dist.email && h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "rgba(245,240,232,.5)" } }, dist.email),
                      dist.phone && h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "10px", color: "rgba(245,240,232,.5)" } }, dist.phone),
                    ),

                    dist.products && h("div", { style: { ...S.dim, marginBottom: "8px", fontStyle: "italic" } }, dist.products),

                    h("div", { style: S.row },
                      h("button", {
                        style: S.btnSm,
                        onClick: () => setExpandedEmail((prev) => ({ ...prev, [emailKey]: !isEmailOpen })),
                      }, isEmailOpen ? "Hide Email" : "Preview RFQ Email"),
                      dist.email && h("a", {
                        href: "mailto:" + dist.email + "?subject=" + encodeURIComponent("RFQ – " + (FSC_NAMES[fsc] || fsc) + " | Government Requirement | Imperio Federal Logistics") + "&body=" + encodeURIComponent(emailText),
                        style: { ...S.btn, textDecoration: "none", fontSize: "9px", padding: "4px 12px" },
                      }, "✉ Open in Mail"),
                      h("button", {
                        style: { ...S.btnSm, color: isCopied ? "#2ecc71" : undefined, borderColor: isCopied ? "rgba(46,204,113,.4)" : undefined },
                        onClick: () => handleCopy(emailKey, emailText),
                      }, isCopied ? "✓ Copied" : "Copy Email"),
                      h("button", {
                        style: { ...S.btnSm, color: "rgba(46,204,113,.8)", borderColor: "rgba(46,204,113,.3)" },
                        onClick: () => { if (confirm("Mark RFQ to " + dist.name + " as sent?")) handleMarkSent(fsc, dist, sols); },
                      }, "✓ Mark Sent"),
                    ),

                    // Email preview
                    isEmailOpen && h("div", { style: S.emailBox }, emailText),
                  );
                }),
              ),
            );
      ...
      //  BLAST LOG VIEW
      // ═══════════════════════════════════════
      activeView === "log" && h("div", null,

        h("div", { style: { ...S.row, marginBottom: "14px" } },
          h("div", { style: { ...S.cardTitle, margin: 0 } }, "Blast Log — " + blastLog.length + " entries"),
          blastLog.length > 0 && h("button", { style: S.btnDanger, onClick: handleClearLog }, "Clear Log"),
        ),

        blastLog.length === 0 && h("div", { style: { ...S.dim, padding: "24px 0", textAlign: "center" } }, "No blasts logged yet. Fire some RFQs and mark them sent."),

        blastLog.map((entry) =>
          h("div", { key: entry.id, style: { ...S.card, padding: "12px 16px" } },
            h("div", { style: { ...S.row, marginBottom: "8px" } },
              h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "11px", color: "var(--alabaster,#F5F0E8)", fontWeight: "700" } }, entry.dist_name),
              h("span", { style: S.badge("rgba(52,152,219,.2)") }, "FSC " + entry.fsc),
              h("span", { style: S.badge(entry.quoted ? "rgba(46,204,113,.25)" : "rgba(201,168,76,.15)") }, entry.quoted ? "QUOTED ✓" : "AWAITING"),
              h("span", { style: S.dim }, new Date(entry.sent_at).toLocaleDateString()),
            ),
            ...
            entry.dist_email && h("div", { style: { ...S.dim, marginBottom: "8px" } }, "→ " + entry.dist_email),

            h("div", { style: S.row },
              // Toggle quoted status
              h("button", {
                style: { ...S.btnSm, color: entry.quoted ? "rgba(46,204,113,.8)" : undefined, borderColor: entry.quoted ? "rgba(46,204,113,.4)" : undefined },
                onClick: () => handleLogUpdate(entry.id, "quoted", !entry.quoted),
              }, entry.quoted ? "✓ Quoted" : "Mark Quoted"),

              // Push to pipeline note
              entry.quoted && h("span", { style: { fontFamily: "JetBrains Mono,monospace", fontSize: "9px", color: "#2ecc71", padding: "4px 10px", border: "1px solid rgba(46,204,113,.3)", borderRadius: "3px" } },
                "→ Ready for Pipeline Entry"
              ),
            ),

            // Quote note
            entry.quoted && h("textarea", {
              style: { ...S.textarea, minHeight: "50px", marginTop: "8px", fontSize: "10px" },
              placeholder: "Quote details: price, lead time, COO, contact name...",
              value: entry.quote_note || "",
              onChange: (e) => handleLogUpdate(entry.id, "quote_note", e.target.value),
            }),
          ),
        ),
      ),
