  ...
  try {
    ...
    switch (action) {
      ...
      case "dbRestoreFromArchive": {
        ...
        if (!rec) { result = false; break; }
        const { archived, archive_reason, archive_date, _id, ...restored } = rec;
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ ok: true, result: clean(result) }) };
